...
def main():
    clear()
    setTitle(f"DiscordHack @Kubis Menu v{THIS_VERSION}")
    astraahometitle()
...
if __name__ == "__main__":
    import sys
    setTitle("DiscordHack Premium Loading...")
    ...
    try:
        ...
        input(f"{y}[{Fore.RED}#{y}]{w} Sorry but, your python version ({sys.version_info[0]}.{sys.version_info[1]}.{sys.version_info[2]}) is not compatible with @Kubis, please download python 3.9 or higher.")
        sys.exit()
    else:
        search_for_updates()
        if not os.path.exists(getTempDir()+"\\atio_proxies"):
